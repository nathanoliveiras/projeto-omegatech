...
function Select({name, options, label, contentClass, className, ...rest}) {
    ...
    return (
        <div className={` flex flex-col mb-2 ${contentClass}`}>
            ...
            <select name={name} ref={selectRef} className={` focus:border-blue-500 p-2 border-solid border focus:outline-none block w-full focus:ring focus:ring-blue-500 focus:ring-opacity-50 rounded-md shadow-sm ${className && className} ${error?'border-red-400':'border-gray-300' }` } {...rest} >
